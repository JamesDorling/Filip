...
public class SpeechVolOptions : MonoBehaviour
{
    public Slider audio;
    ...
    void Start()
    {
        ...
        audio.value = PlayerPrefs.GetFloat("sVolume", 0.5f);
        //set the listener for when the inputfield is changed
        audiotxt.onValueChanged.AddListener(delegate { speechChangeCheck(); });
    }
    ...
    void Update()
    {
        ...
        PlayerPrefs.SetFloat("sVolume", audio.value);
    }

    void speechChangeCheck()
    {
        //get the input value
        float i;
